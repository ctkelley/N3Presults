...
function store_basics(n=32,c=.999,tol=1.e-8,maxnl=20)
FV=zeros(n);
JV64=zeros(n,n);
JV=zeros(Float32,n,n);
JV16=zeros(Float16,n,n);
...
function Tabulate_Me(hout64,hout,hout16,houtmp,houtmph, maxnl; 
                   TeX=true, figures=3)
Lmax=maxnl+1
showtime=zeros(Lmax,5); showtime .*= NaN;
n64=length(hout64)
n32=length(hout)
n16=length(hout16)
lmp=length(houtmp)
...
xmp=0:1:lmp-1
xmph=0:1:lmph-1
semilogy(x64,hout64/hout64[1], "k-",
         x32,hout/hout[1], "ko",
         xmp,houtmp/houtmp[1],"k--",
         x16,hout16/hout16[1],"k-.",
         xmph,houtmph./houtmph[1],"k-o")
